...
class SearchTrainInteractor: BaseDataServiceProvider, PresenterToInteractorProtocol {
    ...
    func fetchallStations() {
        if Reach().isNetworkReachable() == true {
            self.fetch(from: "http://api.irishrail.ie/realtime/realtime.asmx/getAllStationsXML") { (result) in
                switch result {
    ...
    func fetchTrainsFromSource(sourceCode: String, destinationCode: String) {
        ...
        if Reach().isNetworkReachable() {
            fetch(from: urlString) { (result) in
                switch result {
                case .success(let data):
    ...
    private func proceesTrainListforDestinationCheck(trainsList: [StationTrain]) {
        ...
        for index  in 0...trainsList.count-1 {
            ...
            if Reach().isNetworkReachable() {
                ...
                fetch(from: _urlString) { (result) in
                    switch result {
                    case .success(let data):
